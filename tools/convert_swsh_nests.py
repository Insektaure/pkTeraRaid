...
Also reads from external/PKHeX_Raid_Plugin for den map coordinates and location names.
Outputs to include/swsh/den_hashes.h, sword_nests.h, shield_nests.h, den_locations.h.
"""
...
# PKHeX Raid Plugin paths
PKHEX_PLUGIN = os.path.join(PROJECT_DIR, "external", "PKHeX_Raid_Plugin", "PKHeX_Raid_Plugin")
NEST_LOCATIONS_CS = os.path.join(PKHEX_PLUGIN, "Nests", "NestLocations.cs")
RAID_PARAMS_CS = os.path.join(PKHEX_PLUGIN, "Raid", "RaidParameters.cs")
...
def parse_location_names(filepath):
    """Parse RaidParameters.cs to extract LocationNames array."""
    with open(filepath, encoding="utf-8") as f:
        content = f.read()

    # Extract strings between the LocationNames braces
    m = re.search(r'LocationNames\s*=\s*\{(.*?)\};', content, re.DOTALL)
    if not m:
        print("ERROR: Could not find LocationNames in RaidParameters.cs", file=sys.stderr)
        return []
...
def parse_nest_locations(filepath, den_count=276):
    """Parse NestLocations.cs to extract (mapX, mapY, location) for each den.
    Returns list of (mapX, mapY, locationId) tuples, length = den_count.
    """
    with open(filepath, encoding="utf-8") as f:
        content = f.read()

    # Match all NestHashDetail constructor calls
    # Pattern: new NestHashDetail(hash1, hash2, location, mapX, mapY)
    pattern = re.compile(
        r'new\s+NestHashDetail\s*\('
        r'\s*0x[0-9a-fA-F]+\s*,'   # CommonHash
        r'\s*0x[0-9a-fA-F]+\s*,'   # RareHash
        r'\s*(\d+)\s*,'             # Location
        r'\s*(\d+)\s*,'             # MapX
        r'\s*(\d+)\s*\)'            # MapY
    )
    ...
    for m in pattern.finditer(content):
        ...
        mx = int(m.group(2))
        my = int(m.group(3))
        entries.append((mx, my, loc))

    if len(entries) < den_count:
        print(f"WARNING: Only found {len(entries)} nest locations, expected {den_count}",
              file=sys.stderr)

    return entries[:den_count]


def generate_den_locations_h(locations, location_names):
    """Generate include/swsh/den_locations.h."""
    lines = [
        "#pragma once",
        "// Auto-generated by tools/convert_swsh_nests.py — do not edit manually",
        "#include <cstdint>",
        ...
        "};",
        "",
        f"inline constexpr int DEN_COUNT = {len(locations)};",
        "",
        f"inline constexpr DenLocation DEN_LOCATIONS[{len(locations)}] = {{",
    ]

    for i, (mx, my, loc) in enumerate(locations):
        region = "WA" if i < 100 else ("IoA" if i < 190 else "CT")
        lines.append(f"    {{{mx:5d}, {my:5d}, {loc:2d}}}, // Den {i:3d} ({region})")

    lines.append("};")
    lines.append("")

    lines.append(f"inline constexpr int LOCATION_COUNT = {len(location_names)};")
    lines.append("")
    lines.append(f"inline constexpr const char* LOCATION_NAMES[{len(location_names)}] = {{")
    for i, name in enumerate(location_names):
        lines.append(f'    "{name}", // {i}')
    lines.append("};")
    lines.append("")

    # Map image dimensions for coordinate-to-screen conversion
    lines.append("// Map image dimensions (pixel space for coordinates)")
    lines.append("inline constexpr int MAP_IMG_W_WA  = 458;")
    lines.append("inline constexpr int MAP_IMG_H_WA  = 1064;")
    lines.append("inline constexpr int MAP_IMG_W_IOA = 1183;")
    lines.append("inline constexpr int MAP_IMG_H_IOA = 1183;")
    lines.append("inline constexpr int MAP_IMG_W_CT  = 1920;")
    lines.append("inline constexpr int MAP_IMG_H_CT  = 2060;")
    lines.append("")

    lines.append("} // namespace SwShDenLocations")
    lines.append("")
...
def main():
    ...
    print("Parsing NestLocations.cs...")
    locations = parse_nest_locations(NEST_LOCATIONS_CS)
    print(f"  {len(locations)} den locations extracted")

    print("Parsing RaidParameters.cs location names...")
    location_names = parse_location_names(RAID_PARAMS_CS)
    print(f"  {len(location_names)} location names")

    print("Generating den_locations.h...")
    locations_h = generate_den_locations_h(locations, location_names)
    with open(os.path.join(OUT_DIR, "den_locations.h"), "w") as f:
        f.write(locations_h)
    ...
    print(f"  den_locations.h:  {len(locations)} dens, {len(location_names)} locations")
